...
const HeaderCpn = ({ isHomepage }) => {
    ...
    return (
        <header className={`${classHeader} ${isFixed ? 'is-sticky' : ''} ${active ? 'menu-fixed' : ''} ${isHomepage ? 'absolute top-[0] left-[50%] -translate-x-[50%] text-fic-white border-fic-white' : 'text-fic-black border-fic-black'} header flex w-full lg:w-[calc(100%-90px)] mx-auto py-[1rem] md:py-[1.563rem] md:px-[1.25rem] px-[0.75rem] z-10 border-b`}>
