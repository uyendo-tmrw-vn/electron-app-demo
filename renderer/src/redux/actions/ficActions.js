...
export const GetPriceTotal = params => async dispatch => {
  ...
  try {
    ...
    api.post('/check-booking', params)
      .then(
        data => dispatch({ type: actionTypes.GET_TOTAL_PRICE_SUCCESS, response: data }),
        error => dispatch({ type: actionTypes.GET_TOTAL_PRICE_ERROR, error: error })
      )
  } catch (error) {
    dispatch({ type: actionTypes.GET_TOTAL_PRICE_ERROR, error: error })
  }
}
