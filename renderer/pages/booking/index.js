...
import { ClearData, GetListStudio, PostBooking, GetListDuration, GetListTimeSlot, GetCalendar, GetListStartTime, CheckPaymentStatus, GetPriceTotal } from '@/redux/actions/ficActions';
...
const BookStudio = () => {
    ...
    */
    ...
    const { loading, DataListStudio, DataBooking, DataListDuration, DataCalendar, DataListStartTime, DataCheckPaymentStatus, DataTotalPrice } = useSelector((state) => state.FicReducers)
        ...
        <div ref={ref} className="CalendarRef">
            <Calendar
                ...
                headerRender={(value, type, onChange, onTypeChange) => {
                    return (CalendarHeader(value, type, onChange, onTypeChange))
                }}
            />
        </div>
    ));
    ...
    */
    const getCalendarByRoomMonth = ({ month, roomId }) => {
        ...
        const xYear = moment(month).year();
        dispatch(GetCalendar({ "month": xMonth, "year": xYear, "room": roomId }))
    }
    ...
    */
    ...
    useEffect(() => {
        // console.log({ DataCheckPaymentStatus });
    ...
    useEffect(() => {
        if (DataListStartTime) {
            if (DataListStartTime.times) {
                setTimeslot({
                    ...timeslot,
                    list: DataListStartTime.times
                })
            }
            if (DataListStartTime.status == false) {
                setTimeslot({
                    ...timeslot,
                    list: []
                })
                notification.error({
                    message: 'Error',
                    description: DataListStartTime.error?.room
                })
            }
        }
    }, [DataListStartTime])
                ...
                <div className="grid grid-cols sm:grid-cols-2 mb-[3.375rem]">
                    <div className="mb-[3.125rem] max-w-[100%] lg:max-w-[90%]">
                        ...
                        <div className="text-[1.375rem] leading-7">
                            ...
                            <p className="mb-3">A minimum booking of 2 hours is required, which can be enjoyed with complimentary use of basic production equipment.</p>
                    ...
                    <div>
                        ...
                        >
                            ...
                            >
                                <Form.Item name='studio' rules={[{ required: true, message: 'Please choose Studio!' }]} className='mb-2 typeAndDate' label="Studio Type">
                                    <Select
                                        ...
                                        dropdownClassName="select-on-bookingpage"
                                ...
                                text-[1.25rem] rounded-[3rem] w-[10rem] h-[3rem]
                            ...
                            >
                                ...
                                {/* form 2 */}
                                <div className={`cart-summary w-full mx-auto pt-3 mt-4 ${listBookingSelected.length > 0 ? 'border-t-2' : ''} `}>
                                    {
                                        ...
                                        <div className="w-full my-3 max-h-[25.4rem] overflow-y-auto">
                                            {listBookingSelected.map((item, index) => {
                                                return (
                                                    <div key={index} className={`${item.invalidBooking ? 'bg-[darkgray]' : ''} relative flex shadow mb-2 bg-[#ddd] rounded-[0.3rem] items-center py-2 leading-normal`}>
